...
- [x] 8.1 The branch `feat/seed-auto-memory-store` already carries `a4deb6e`. Do **not** rebase it onto
      unrelated work; the uncommitted `config.env.example` edit stays out (task 10.1).
- [x] 8.2 `tools/preflight.py .` before the first push (ADR-0041).
- [x] 8.3 `tools/pr-flow.py --plan --branch feat/seed-auto-memory-store`, then drive it; run each
      emitted command **verbatim**.
- [x] 8.4 Archive **on the feature branch** before merge (ADR-0040).
...
## Landing record (PR #102)

- Branch pushed `75356a5`, PR #102 opened, 36 checks green, archived on the branch per ADR-0040 at
  `fe8a2b6` — the deltas synced into both `protects:`-tagged specs at that point, which is when the
  constitutional diff gate became applicable.

**Two defects met on the way, recorded because both cost a round trip.**

1. **`gh pr edit` silently no-ops.** It failed on GitHub's Projects-classic GraphQL deprecation and
   changed nothing, while reporting nothing actionable. The body was verified unchanged by an
   anonymous REST read. `gh api -X PATCH /repos/OWNER/REPO/pulls/N -F body=@FILE` is the working
   route. `gh pr *` forms are on the vault's deny list precisely for this; one should never have been
   composed, in a handover block or anywhere else.

2. **The declared-scope gate reads a push-time snapshot.** The `pull_request` event payload carries
   the body as of the push, so correcting a body and re-running the job replays the STALE body. A
   body fix requires a **new push**, never a re-run. The archive directory name is generated by
   `openspec archive` from **UTC** — predicting it in a scope block instead of reading it back is what
   made the correction necessary at all.
