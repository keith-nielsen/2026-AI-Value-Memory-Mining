...
- [ ] 1.1 `maintenance` ADDED: *An Outward Command Is Checked Against The Driver's Emission* (6 scenarios)
- [ ] 1.2 `maintenance` ADDED: *A Downgrade Record Is Forgeable And Says So* (2 scenarios)
...
- [ ] 2.1 Define the record: `command` (verbatim), `step`, `branch`, `expires`. Written to
      `.git/pr-flow/emitted.json`, beside the saved plan, reusing `PLAN_TTL_SECONDS`
- [ ] 2.2 `tools/pr-flow.py` `emit()` (`:498`) writes it for **every** step, not only
      `runs == OPERATOR` — the asymmetry this change exists to correct
- [ ] 2.3 `discard_saved_plan()` also discards the emission record at LIFECYCLE COMPLETE. **A record
      that outlives its step is an authorisation left on the desk** — the item-20 shape
- [ ] 2.4 `tools/ship-release.py` writes records through the same helper (design decision 2), else
...
- [ ] 3.1 Generalise `_targets_vault(cmd, cwd)` → `_zone(cmd, cwd)` returning `vault | governed |
...
- [ ] 3.2 `vault` → existing HARD DENY, unchanged. `elsewhere` → existing ASK, unchanged
- [ ] 3.3 `governed` + byte-identical live record for the current branch → **ALLOW**, printing the
      step that matched (design decision 1)
- [ ] 3.4 `governed` + no match → **ASK plus a diff naming what differs** — not merely "mismatch"
- [ ] 3.5 **The invariant, as code and as a test:** no path added here returns `deny`. Absent,
      expired, wrong-branch, unparseable and comparison-fault all fall through to ASK
- [ ] 3.6 Stdlib only, no network, no `subprocess` — the guard is AST-analysed by
...
- [ ] 3.7 Render and reconcile: `vault-render.py render`, then `reconcile` reports zero drift
...
- [ ] 4.1 No record → **ASK** (not deny)
- [ ] 4.2 Expired record → **ASK**
- [ ] 4.3 Record for another branch → **ASK**
- [ ] 4.4 Corrupt / unparseable record → **ASK** (fail toward the prompt, never toward allow or deny)
- [ ] 4.5 Mangled command with a live record → **ASK**, and the diff names the cause. Fixture: the
      real 2026-08-16 command, `R=…; cd "$R"; timeout 180 git -C "$R" p[u]sh …`
- [ ] 4.6 Verbatim command with a live record → **ALLOW**, naming the step
- [ ] 4.7 Vault target **with** a valid-looking record → still **DENY**. The record must not be able
      to unlock INV-14; this is the test that proves the downgrade is scoped
- [ ] 4.8 Ungoverned repository → behaviour byte-identical to today, record or no record
- [ ] 4.9 `inv6-offline-check.py` still passes on the note; `tests/test_inv6_offline.py` still passes
- [ ] 4.10 Every test above observed **failing without the change** before its box is ticked

## 5. Documents this makes true or false

- [ ] 5.1 The script note's Rationale describes the third zone and **states the record is forgeable**
- [ ] 5.2 ADR-0043 — context / options / choice / consequence, citing ADR-0018's tripwire posture and
      ADR-0027's effective-target refinement
- [ ] 5.3 `CONTRIBUTING.md` — one line under *Landing a change*: run the emitted command verbatim;
      deviation now prompts with a diff
- [ ] 5.4 Verify no document claims the mechanism authorises or verifies a command. It matches a
...
- [ ] 7.1 `python3 tools/preflight.py .` CLEAR before any push
